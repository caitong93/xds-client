// This file is coped from istio with modifications.
...
type Config struct {
	...
	// Backoff specifies grpc backoff config. Defaults to nil, no backoff.
...
func (a *ADSC) Run() error {
	...
	if len(a.certDir) > 0 {
		...
		if a.backoff != nil {
			opts = append(opts, grpc.WithConnectParams(grpc.ConnectParams{Backoff: *a.backoff}))
		}
	...
	edsstr, err := xds.StreamAggregatedResources(context.Background(), callOpts...)
	if err != nil {
		return err
	}
	a.stream = edsstr
...
func (a *ADSC) initNode(opts *Config) {
	if opts.Namespace == "" {
	...
	n := &core.Node{
		Id: fmt.Sprintf("%s~%s~%s.%s~%s.svc.cluster.local", opts.NodeType, opts.IP,
